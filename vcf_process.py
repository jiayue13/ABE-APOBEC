...
- 支持通过 --basename 区分 ProQ-ABE / ProQ-APOBEC 等不同前缀
"""
...
def parse_args():
    ...
    ap.add_argument("--input", required=True,
                    help="Input VCF directory (containing {basename}_FWD/REV.vcf[.gz])")
    ap.add_argument("--output", required=True, help="Output Directory")
    ap.add_argument(
        "--basename",
        default="ProQ-ABE",
        help="Base name of VCF file, e.g. 'ProQ-ABE' or 'ProQ-APOBEC'. "
             "Input VCF will be {basename}_FWD/REV.vcf"
    )
    ap.add_argument("--samples", nargs="*", default=None,
                    help="Optional: list of sample names; if not provided, they will be "
                         "automatically inferred from the VCF header")
...
def main():
    ...
    # Input file: {basename}_FWD/REV.vcf(.gz)
    vcf_path = os.path.join(input_dir, f"{basename}_{st}.vcf")
    vcf_gz = vcf_path + ".gz"
    ...
    if args.samples and len(args.samples) > 0:
        if len(args.samples) != len(sample_cols_from_header):
            print("[WARN] The number of --samples is inconsistent with the number of VCF sample columns. "
                  "The VCF header will take precedence.")
    ...
    for sample_col, sp in zip(sample_cols, sList):
        ...
        with np.errstate(divide="ignore", invalid="ignore"):
            df[f"{sp}.ratio"] = (
                df[f"{sp}.AD"].astype(float) /
                df[f"{sp}.DP"].astype(float)
    ...
    # === Write out the file ===
    out_path = os.path.join(output_dir, f"mpileup_fixstrand_{basename}_{strand}.vcf")
    out_path = os.path.normpath(out_path)
    print("[INFO] Target output path:", out_path)
